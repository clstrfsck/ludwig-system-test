...
def test_no_leading_parameter():
    simple_edit_test("ep/m=(1,11)/yr", " abc def\nghi  jkl\n", "    abc def\nghi  jkl\n")

def test_plus_leading_parameter():
    simple_edit_test("ep/m=(2,12)/+yri/m1/", "  abc def\nghi  jkl\n", "     abc def\ngm1hi  jkl\n")

def test_positive_leading_parameter():
    words = "  abc def ghi jkl mno pqr\n"
    unaligned = words * 4
    aligned = ("       abc def ghi jkl mno pqr\n" * 2) + (words * 2)
    simple_edit_test("ep/m=(2,30)/2yr", unaligned, aligned)
...
def test_eop_leading_parameter():
    ...
    words = (line * 4) + "\n" + (line * 2)
    aligned = (aline * 4) + "\n" + (line * 2)
    simple_edit_test("ep/m=(3,33)/>yr", words, aligned)
